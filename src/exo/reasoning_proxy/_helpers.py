"""Typed helpers for narrowing `object`-typed JSON values.

The proxy is a thin middleware that inspects upstream JSON without knowing the
full schema of every model's extensions. We use `object` (not `Any`) everywhere
and narrow with these helpers so basedpyright stays strict.
"""
