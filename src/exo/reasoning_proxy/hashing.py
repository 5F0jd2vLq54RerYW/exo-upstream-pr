...
def _canonical_tool_calls(tool_calls: list[dict[str, object]] | None) -> list[dict[str, object]]:
    if not tool_calls:
        return []
...
def hash_openai_assistant(
    ...
    payload = json.dumps(shape, sort_keys=True, ensure_ascii=False, separators=(",", ":"))
...
def hash_claude_assistant(content_blocks: list[dict[str, object]]) -> str:
    ...
    for block in content_blocks:
        ...
        if btype == "text":
            normalized.append({"type": "text", "text": dict_get_str(block, "text") or ""})
        elif btype == "tool_use":
            normalized.append({
                "type": "tool_use",
