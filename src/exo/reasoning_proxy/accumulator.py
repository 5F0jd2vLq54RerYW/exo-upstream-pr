...
class OpenAIAccumulator:
    ...
    def _consume_event(self, event: dict[str, object]) -> None:
        ...
        for choice_raw in choices:
            ...
            if not (isinstance(index_val, int) and not isinstance(index_val, bool)) or index_val != 0:
                continue
    ...
    def tool_calls(self) -> list[dict[str, object]] | None:
        if not self._tool_calls_by_index:
            return None
        ordered = [self._tool_calls_by_index[i] for i in sorted(self._tool_calls_by_index)]
        return ordered
